...
function placeholder(name){nav(name);app.innerHTML=`<section class="page placeholder"><h2>${name[0].toUpperCase()+name.slice(1)}</h2><p>This area stays intact for the next build step.</p></section>`}
...
document.querySelectorAll("[data-nav]").forEach(b=>b.onclick=()=>b.dataset.nav==="household"?household():placeholder(b.dataset.nav));
household();
